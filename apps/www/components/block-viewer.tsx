...
function BlockViewerProvider({
  ...
  highlightedFiles,
  children,
}: Pick<BlockViewerContext, "item" | "tree" | "highlightedFiles" | "name"> & {
...
function BlockViewerToolbar() {
  const { setView, view, item, resizablePanelRef, setIframeKey, name } =
    useBlockViewer();
...
function BlockViewerIframe({ className }: { className?: string }) {
  const { iframeKey, name } = useBlockViewer();
...
function BlockViewerMobile({ children }: { children: React.ReactNode }) {
  const { item, name } = useBlockViewer();
...
function Tree({ item, index }: { item: FileTree; index: number }) {
  const { activeFile, setActiveFile, name } = useBlockViewer();
...
function BlockCopyCodeButton() {
  const { activeFile, item, name } = useBlockViewer();
  const { copyToClipboard, isCopied } = useCopyToClipboard();
