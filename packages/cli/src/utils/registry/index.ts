...
export async function getRegistryBaseColor(baseColor: string) {
  try {
    const [result] = await fetchRegistry([`colors/${baseColor}.json`], shadcnBaseUrl)
...
export async function resolveTreeWithShadcn(
    ...
    if (!calledByShadcn) {
      ...
      if (!entry) {
        ...
        if (shadcnEntry.registryDependencies) {
          const { shadcnTree: shadcnTreeDependencies } = await resolveTreeWithShadcn(shadcnIndex,
            index,
      ...
      if (entry && entry.registryDependencies) {
        ...
        const { magicuiTree: magicuiTreeDependencies, shadcnTree: shadcnTreeDependencies } = await resolveTreeWithShadcn(shadcnIndex,
          index,
  ...
  return {
    shadcnTree: shadcnTree.filter((component, index, self) => self.findIndex((c) => c.name === component.name) === index),
    magicuiTree: magicuiTree.filter((component, index, self) => self.findIndex((c) => c.name === component.name) === index),
  }
}
...
export async function fetchTreeFromShadcn(
  style: string,
  tree: theTree
) {
  try {
